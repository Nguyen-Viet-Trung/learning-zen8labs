...
public class StringUtil {
    ...
    public static String applySha256(String input) {
        try{
            MessageDigest digestM = MessageDigest.getInstance("SHA-256");
            byte[] hash = digestM.digest(input.getBytes("UTF-8"));
    ...
    public static boolean verifyECDSASignature(PublicKey publicKey, String data, byte[] signature){
        try{
            Signature ecdsaVerify = Signature.getInstance("ECDSA", "BC"); //BC = Bounty Castle
